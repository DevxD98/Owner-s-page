...
import { ChevronRight, Sparkles, Plus, Search, Filter, Tag } from 'lucide-react';
...
const OfferManagement = ({ 
  ...
  return (
    <div className={`mt-4 sm:mt-6 transition-all duration-500 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      ...
      >
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <h2 className="text-lg sm:text-xl font-bold text-gray-800 group-hover:text-teal-600 transition-colors">Recent Live Offers</h2>
        </div>
        {!showAllItems && (
          <ChevronRight size={20} className="text-gray-500 group-hover:text-teal-600 transform group-hover:translate-x-1 transition-transform" />
        )}
      </div>
          ...
          <div className="flex items-center gap-2 mb-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="pl-9 sm:pl-10 w-full p-2 sm:p-2.5 text-xs sm:text-sm border border-gray-300 rounded-lg shadow-sm focus:ring-teal-500 focus:border-teal-500"
                value={searchTerm}
            ...
            >
              <Filter size={18} className={showFilters ? "text-teal-600" : "text-gray-500"} />
            </button>
          </div>
      ...
      <div className="space-y-4 sm:space-y-5">
        {publishedOffers && publishedOffers.length > 0 ? (
          ...
          <div className="text-center py-6 sm:py-8 border-2 border-dashed border-gray-200 rounded-lg">
            <div className="mx-auto w-12 h-12 sm:w-16 sm:h-16 mb-3 sm:mb-4 rounded-full bg-gray-100 flex items-center justify-center">
              <Tag size={20} className="sm:size-24 text-gray-400" />
            </div>
            ...
            {!searchTerm && filter === 'all' && (
              <button 
                ...
                className="mt-3 sm:mt-4 px-3 sm:px-4 py-1.5 sm:py-2 bg-teal-500 text-white rounded-lg hover:bg-teal-600 transition-colors flex items-center mx-auto text-xs sm:text-sm"
              >
        ...
        {offers && offers.filter(o => !o.isDraft).length > 2 && !showAllItems && (
          <button 
            ...
            className="w-full py-1.5 sm:py-2 mt-2 text-teal-600 bg-teal-50 rounded-lg text-center text-xs sm:text-sm font-medium transition-colors hover:bg-teal-100"
          >
