...
import { Edit, ExternalLink, Calendar, ToggleLeft, ToggleRight } from 'lucide-react';
import { useApp } from '../../contexts/AppContext';
import { useNavigate } from 'react-router-dom';

const OfferItem = ({ id, title, validTill, isActive, description, image, isDraft }) => {
  ...
  return (
    <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm transition-all duration-300 hover:shadow-md group">
      <div className="flex items-start gap-3">
        ...
        <div className="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
          {image ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200">
              <span className="text-gray-400 text-xs font-medium">{title.substring(0, 2).toUpperCase()}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex-1">
          <div className="flex justify-between">
            <h3 className="font-semibold text-gray-800 group-hover:text-amber-600 transition-colors">{title}</h3>
