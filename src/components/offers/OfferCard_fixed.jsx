...
const isMobileBrowser = () => {
  if (typeof window !== 'undefined') {
    ...
    // Enhanced mobile detection regex including tablets
...
const OfferCard = ({
  ...
  const swipeableHandlers = useSwipeable({
    ...
    swipeDuration: 300,
    onSwiping: (data) => {
      // Only apply desktop logic when not on mobile
      if (isMobile) return;
      
      // Desktop-only logic since mobile handlers are disabled
      // First check: if there's ANY vertical movement at the start of the gesture,
      // treat it as a scroll and ignore completely
      if (Math.abs(data.deltaY) > 10 && Math.abs(data.deltaX) < 40) {
        return;
      }
      ...
      const isVerticalScroll = Math.abs(data.deltaY) > Math.abs(data.deltaX) * 0.8;
      if (isVerticalScroll) {
        if (swipeOffset !== 0) {
          animateSpringBack();
        }
      ...
      if (data.dir === 'Left') {
        ...
        // Desktop logic for auto-delete
        const velocityThreshold = 1.2;
        if (newOffset < deleteThreshold && Math.abs(data.velocity) > velocityThreshold) {
    ...
    onSwipedLeft: (data) => {
      ...
      const isSignificantHorizontalSwipe = Math.abs(data.deltaX) > 40;
      const isVerticalScroll = Math.abs(data.deltaY) > Math.abs(data.deltaX) * 0.8;
      ...
      // Only delete on very deliberate swipes
      const velocityThreshold = 1.0;
      const distanceThreshold = -140;
      if (isSignificantHorizontalSwipe && data.velocity > velocityThreshold && data.deltaX < distanceThreshold) {
        animateDelete();
        return;
      }
